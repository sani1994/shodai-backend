...
def send_summary_email():
    ...
    orders_daily = Order.objects.filter(placed_on__gte=today).exclude(order_status='CN')
    ...
    for user in users_daily:
        completed_order = Order.objects.filter(placed_on__lt=today, user=user).exclude(order_status='CN').count()
        if completed_order:
            repeat_customer_daily += 1
    orders_monthly = Order.objects.filter(placed_on__gte=this_month).exclude(order_status='CN')
    users_monthly = set([order.user for order in orders_monthly])
    for user in users_monthly:
        completed_order = Order.objects.filter(placed_on__lt=this_month, user=user).exclude(order_status='CN').count()
        if completed_order:
            repeat_customer_monthly += 1
    all_user = UserProfile.objects.filter(user_type='CM', is_active=True, is_approved=True)
    for user in all_user:
        completed_order = Order.objects.filter(user=user).exclude(order_status='CN').count()
    ...
    payload = {"new_client_daily": UserProfile.objects.filter(created_on__gte=today).count(),
               ...
               "order_placed_daily": orders_daily.count(),
               ...
               "order_placed_monthly": orders_monthly.count(),
