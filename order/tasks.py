...
def send_summary_email():
    ...
    payload = {"new_client_daily": UserProfile.objects.filter(created_on__gte=today).count(),
               ...
               "order_daily": Order.objects.filter(placed_on__gte=today).exclude(order_status='CN').count(),
               ...
               "order_monthly": Order.objects.filter(placed_on__gte=this_month).exclude(order_status='CN').count(),
               ...
               "total_order": Order.objects.all().exclude(order_status='CN').count(),
